...
func helpTopics(p string) []helpTopic {
	return []helpTopic{
		...
		{"cats", "cats, breeding and cat events", []string{
		...
		{"stealing", "robbing others and defending yourself", []string{
			...
			"Wear armor (helmet, gloves, vest) with " + p + "equip to defend. Thieves only reach loose ohayous, so bank the rest: " + p + "use vault, then " + p + "deposit and " + p + "withdraw (once a day).",
